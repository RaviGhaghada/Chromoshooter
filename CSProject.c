...
bool updateString (Event &event, string &str)
{
    ...
    else if (ch==8 && str.size()!=0) // BACKSPACE
        str.pop_back();
...
void addScoreRecord (Score x)
{/*
    Score temp;
    fstream fo;
    fstream fi (savedata, ios::in | ios::binary);

    if (!isEmpty(fi))
    {
        fo.open("temp.dat", ios::app | ios::binary);
        while (!fi.eof())
        {
            fi.read((char*)& temp, sizeof(Score));
            if (temp.score>=x.score)
                fo.write((char*)&temp, sizeof(Score));
            else
            {
                fo.write((char*)&x, sizeof(Score));
                x.score=-1; // to make "if" condition always true
            }
        }
        ...
        remove(stringToArr(savedata));
        cout << rename("temp.dat", "savedata.dat");
    }
    else
    {
        fo.open (savedata, ios::app | ios::binary);
        fo.write((char*)&x, sizeof(Score));
        fo.close();
        fi.close();
    }
    */
    ...
    fstream fs(savedata, ios::app | ios::binary);
    fs.write((char*)&x, sizeof(Score));
    fs.close();
}
/*
    ---------------------------------------------------------------
...
int introScreen(RenderWindow& window)
{
    ...
    int var=rand()%5;
    Text title ("CHROMO SHOOTER\n  RAVI GHAGHADA", fnt, 3*block);
    title.setFillColor(Color::White);
    int pos=-150;

    Clock time_check;
    while (time_check.getElapsedTime().asSeconds()<10)
    {
        window.clear(Color::Black);

        title.setPosition(pos, videom.height/2-title.getLocalBounds().height/2);
        pos+=block/4;
        ...
        if (title.getPosition().x>videom.width+100)
            break;
    }
            ...
            for (int j=0; j<vmeteors.size(); j++)
                if (areColliding(vmeteors[j].getGlobalBounds(), bullets[i].getGlobalBounds()))
                {
                    if (vmeteors[j].getColor()==bullets[i].getColor())// if color is matching
                    {
                        ...
                        spawnmax = 2000-1000*atan(score/100)*cos(0.2617*score/10);
...
int scoreBoard(RenderWindow& window)
{
    ...
    if (!isEmpty(fs))
    {
        while (fs.read((char*)& tempscore, sizeof(Score)))
            tscores.push_back(tempscore);
        sort(tscores.begin(), tscores.end(), lowerScore);
    }
    fs.close();
